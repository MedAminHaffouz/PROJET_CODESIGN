#
# Matrix Multiplication: Wider Data-Types (myGEMM4)
#
from helper import *
...
# --------------------------------------------------------------------------------
kernel_name = "C_myGEMM4.cl"

print (f"Matrix multiplication {N}*{N} repeated {COUNT} times")
print (f"Executing kernel: {kernel_name} (Wider Data-Types / float4)\n")
...
for i in range(COUNT):    
    try:
        # CRITICAL CHANGE FOR VECTORIZATION: 
        # Vector width (WIDTH) is 4. We use float4 data types.
        # We must reduce the X dimension mapping by a factor of 4.
        WIDTH = 4
        
        global_dim = (int(N/WIDTH), N)
        local_dim = (int(localsize/WIDTH), localsize)
